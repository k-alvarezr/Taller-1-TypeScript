...
let series = [
  ...
  export const listaSeries = new Series(series);

  let seriesTable: HTMLElement = document.getElementById ("series")!;
  let promedioTable: HTMLElement = document.getElementById ("promedio")!;

  mostrarSeries(listaSeries);
  mostrarPromedio(listaSeries);


  function mostrarSeries(Series: Series):void{
      let seriesTbody: HTMLElement = document.createElement("tbody");
      for (let serie of Series.series)
      {
  ...
  function mostrarPromedio(Series: Series):void{
    let promedio: number = Series.darPromedioSeasons();
    let trElement: HTMLElement = document.createElement("tr");
    trElement.innerHTML= `<td><b>Average Seasons</b></td><td>${promedio}</td>`;
